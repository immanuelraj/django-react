...
from .serializers import CategorySerializer, ProductSerializer, ShopSerializer, CategoryListSerializer,/
    ProductListSerializer, ShopListSerializer
...
class ShopViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        page = self.paginate_queryset(queryset)
        if page is not None:
            serializer = ShopListSerializer(page, many=True)
            return self.get_paginated_response(serializer.data)
    ...
    def retrieve(self, request, ext_id):
        try:
            shop = Shop.objects.get(ext_id=ext_id)  
            data = ShopListSerializer(shop).data
            return Response(data, status=status.HTTP_200_OK)
        except Exception as e:
            return Response({"status": "error", "response": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def update(self, request, ext_id):
        try:
            req_data = self.request.data
            shop = Shop.objects.get(ext_id=ext_id)
            old_data = deepcopy(self.serializer_class(shop).data)
            serializer = self.serializer_class(shop, data=req_data, partial=True, context={'request': request, 'old_data': old_data})
            if serializer.is_valid():
                try:
                    with transaction.atomic():
                        serializer.save(force_update=True)
